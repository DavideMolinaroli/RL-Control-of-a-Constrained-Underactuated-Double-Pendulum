...
from utils.reset_functions import zero_reset_func
...
model_path = f"trained_models/model_penalty_pi6/best_model"
...
while not (truncated or done):
    ...
    r = np.round(reward, 2)
    data = [
        ["Step","th1", "th2", "Torque τ [Nm]", "Angular Velocity ω [rad/s]", "Reward"],
        [i, theta1, theta2, tau, omega, r],
    ]
...
# plot_angles(thetas1, thetas2, dt, t_final, model_path)
plot(thetas1, thetas2, omegas1, omegas2, torques, rewards, env.max_V+8, dt, t_final, model_path)
# rewards_plot(rewards, robot, env.max_V + 8, dt, t_final, model_path)

print(
    f"Swingup time: {get_swingup_time(T, np.array(X), mpar=mpar, has_to_stay=True, height=0.95)}s"
)