...
from simulation.visualization import get_arrow, set_arrow_properties
from utils.filters.kalman_filter import kalman_filter_rt
from utils.filters.low_pass import lowpass_filter_rt
from utils.filters.unscented_kalman_filter import unscented_kalman_filter_rt
...
class Simulator:
    ...
    def reset(self):
        ...
        self.meas_noise_vfilter_args = {"alpha": [1.0] * self.plant.dof * 2}
    ...
    def step(self, tau, dt, integrator="runge_kutta"):
        ...
        self.x = np.random.normal(
            self.x, self.process_noise_sigmas
        )  # , np.shape(self.x))
    ...
    def get_real_applied_u(self, u):
        ...
        for i in range(self.plant.dof):
            nu[i] = np.clip(
                nu[i], -self.plant.torque_limit[i], self.plant.torque_limit[i]
            )
    ...
    def rollout(self, x0, policy, T, dt, noise, animate=False):
        ...
        return (
            np.asarray(self.t_values),
            np.asarray(self.meas_x_values),
            np.asarray(self.tau_values),
            np.asarray(self.x_values)[:-1, :],
        )
